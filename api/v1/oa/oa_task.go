...
func (u *TaskApi) GetTaskList(ctx *gin.Context) {
	...
	if err != nil {
		global.MEIS_LOGGER.Error("获取任务列表参数错误", zap.Error(err))
		response.FailWithMessage("获取任务列表参数错误", ctx)
		return
	}
...
func (u *TaskApi) GetTaskInfo(ctx *gin.Context) {
	...
	err := ctx.ShouldBindJSON(&info)
	if err != nil {
		global.MEIS_LOGGER.Error("获取任务参数错误", zap.Error(err))
		response.FailWithMessage("获取任务参数错误", ctx)
		return
	}
...
func (u *TaskApi) CreateTask(ctx *gin.Context) {
	...
	if err != nil {
		global.MEIS_LOGGER.Error("修改任务错误", zap.Error(err))
